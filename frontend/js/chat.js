...
document.getElementById("logoutBtn")?.addEventListener("click", () => {
    swal({
        title: "Are you sure?",
        text: "You are going to logout if confirmed!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
    }).then((willLogout) => {
        if (willLogout) {
            swal("Poof! Good to see you, you can login anytime!", {
                icon: "success",
            }).then(() => {
...
function setSeenIndicator(seenEl, status) {
    if (!seenEl) return;
    seenEl.innerText = "✔✔";
    seenEl.style.color = status === 'read' ? "blue" : "gray";
}
...
function updateMessageStatus(messageId, status) {
    ...
    setSeenIndicator(seenIndicator, status);
    if (status === 'read') {
...
socket.on("update-message-status", ({ senderId }) => {
    ...
    allSent.forEach(div => {
        ...
        if (seen.style.color !== "blue") {
            setSeenIndicator(seen, 'delivered');
        }
    });
});
